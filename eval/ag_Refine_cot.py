...
filename = f'new_data_test/Refine_cot.json'
...
def process_comment(comment, index):
    ...
    comment["response0"] = comment_text
    print(f"data{index} finish response")
    for attempt in range(3):
        try:
            comment_text = ag.call_api2(ap.Refine,RW_input2(comment),model,client1)
            #print(comment_text)
            break
        except Exception as e:
            if attempt < 2:
    ...
    print(f"data{index} finish refined")
...
"""all_data = []
for i in range(29):
    with open(f"refine/refine{i}.json", 'r',encoding='utf-8') as file:
        part = json.load(file)
    all_data += part
...
print(f"成功抽取{len(data)}条数据")
del all_data"""
with open("new_data/new_data_final.json", 'r', encoding='utf-8') as file:
    data = json.load(file)
